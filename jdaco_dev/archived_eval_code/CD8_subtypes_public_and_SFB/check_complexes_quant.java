...
public class check_complexes_quant {
	...
	public static void N_MNP_TFcombinations() {
		...
		for (HashSet<String> TFvariant:TFvariants) {
			...
			if (pm < 0.05)
				System.out.println(DataQuery.batchHGNCNamesFromProteins(TFvariant) + " : " + pm + "  -> " + N_TFV_abundance.get(TFvariant) + " vs " + TMNP_TFV_abundance.get(TFvariant));
		}
	}
	...
	public static void MNP_ALL_TFcombinations() {
		...
		for (HashSet<String> TFvariant:TFvariants) {
			...
			if (pm < 0.05) {
				...
				System.out.println(DataQuery.batchHGNCNamesFromProteins(TFvariant) + " : " + pm + "  -> " + goa.rateCollectionOfProteins(complexes) + " " + TMNP_TFV_abundance.get(TFvariant) + " vs " + other_TFV_abundance.get(TFvariant));
			}
		}
	}
	...
	public static void MNP_ALL_TFcombinationsCleaned() {
		...
		for (HashSet<String> TFvariant:TFvariants) {
			...
			if (pm < 0.05) {
				...
				System.out.println(direction + " " + DataQuery.batchHGNCNamesFromProteins(TFvariant) + " : " + pm + "  -> " + goa.rateCollectionOfProteins(complexes) + 
						" -> " + test_median + " vs " + other_median + " , " + 
						complexes.stream().map(s->DataQuery.batchHGNCNamesFromProteins(s)).collect(Collectors.toList()) + " , " + 
