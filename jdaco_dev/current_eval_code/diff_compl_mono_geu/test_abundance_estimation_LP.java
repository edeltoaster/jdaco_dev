...
public class test_abundance_estimation_LP {
		...
		for (double std:stds)
			for (double prefactor:prefactors) {
				...
				for (Entry<String, String> sample : data.entrySet()) {
					for (int i = 1; i < no_iterations+1; i++) {
						double[] result = simulate_sample_model_run(sample.getKey(), sample.getValue(), std, prefactor, i, sample_construction_outputs);
						all_iterations.add(sample + " " + std + " " + prefactor + " " + i + " " + result[0] + " " + result[1] + " " + result[2] + " " + result[3]);
					}
				}
				...
				Utilities.writeEntries(sample_construction, "sample_construction_LP.txt.gz");
			}
	...
	public static void benchmark2() {
		...
		for (double prefactor:prefactors) {
			...
			for (Entry<String, String> sample : data.entrySet()) {
				for (int i = 1; i < no_iterations+1; i++) {
					double[] result = simulate_sample_model_run2(sample.getKey(), sample.getValue(), prefactor, i, sample_construction_outputs);
					all_iterations.add(sample + " " + std + " " + prefactor + " " + i + " " + result[0] + " " + result[1] + " " + result[2] + " " + result[3]);
				}
			}
			sample_construction.addAll(sample_construction_outputs);
			...
			Utilities.writeEntries(sample_construction, "sample_construction_LP2.txt.gz");
		}
