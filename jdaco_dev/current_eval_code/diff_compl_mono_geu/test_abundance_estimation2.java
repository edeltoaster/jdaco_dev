...
public class test_abundance_estimation2 {
	...
	/**
	 * Realistic simulation of the "equal distribution"-model on the basis of real data and noise regarding the equality of the distribution of abundance values. 
	 * Remaining values are sampled from real data.
	 * @param daco_outfile
	 ...
	 * @return
	 */
	public static Object[] simulate_sample_model(String daco_outfile, String major_transcript_file, double std_factor) {
		...
		// sum up transcript abundances -> values that are AT LEAST necessary
	 ...
	 * @return
	 */
	public static double[] simulate_sample_model_run(String daco_result_file, String major_transcripts_file, double std_factor, int iteration, String[] sample_construction_outputs) {
		
		// get results of simulation
		Object[] simulation = simulate_sample_model(daco_result_file, major_transcripts_file, std_factor);
		...
		if (sample_construction_outputs != null) {
			String out = daco_result_file + " " + std_factor + " " + (iteration+1) + " " + lim_distr_mean + " " + lim_distr_std;
	...
	public static void benchmark() {
		...
		all_iterations.add("sample std iter corr_compl rmsd_compl corr_rem rmsd_rem");
		sample_construction.add("sample std iter lim_distr_mean lim_distr_std");
		
		int no_iterations = 20;
		double[] stds = new double[]{0.01, 0.25, 0.5, 0.75, 1.0, 1.25};
		
		System.out.println("Running benchmark2 on M/GEU");
		...
		for (double std:stds) {
			System.out.println("Running " + std);
			System.out.flush();
			...
			for (Entry<String, String> sample : data.entrySet()) {
				ForkJoinTask<List<double[]>> task = pool.submit(() -> IntStream.range(0, no_iterations).boxed().parallel().map(d -> simulate_sample_model_run(sample.getKey(), sample.getValue(), std, d, sample_construction_outputs)).collect(Collectors.toList()));
			...
			for (String sample : tasks.keySet()) {
				...
				for (double[] result:results) {
					all_iterations.add(sample + " " + std + " " + n + " " + result[0] + " " + result[1] + " " + result[2] + " " + result[3]);
