...
public class test_abundance_estimation {
	...
	/**
	 * Determines a random distribution
	 * @param size
	 * @return
	 */
	public static ArrayList<Double> getRndDistr(int size) {
		ArrayList<Double> distr = new ArrayList<Double>(size);
		ArrayList<Double> cuts = new ArrayList<Double>(size);
		IntStream.range(0, size-1).forEach(c -> cuts.add(rnd.nextDouble()));
		cuts.add(1.0);
		cuts.sort(null);
		distr.add(cuts.get(0));
		for (int i = 1; i < cuts.size(); i++) {
			distr.add(cuts.get(i) - cuts.get(i-1));
		}
	...
	/**
	 * Realistic simulation of the "rnd distribution"-model and noise regarding the portion of remaining protein abundances.
	 * @param daco_outfile
	 * @param major_transcript_file
	 * @param remaining_prefactor
	 * @return
	...
	public static Object[] simulate_sample_model3(String daco_outfile, String major_transcript_file, double remaining_prefactor) {
		...
		for (String limiting_protein:limiting_protein_to_complex.keySet()) {
			...
			// determine distribution from real values
			List<Double> abundances = new ArrayList<>(size);
			ArrayList<Double> distr = getRndDistr(size);
			for (int i = 0; i < size; i++) {
				abundances.add(prot_abundance * distr.get(i));
			}
	...
	public static void benchmark3() {
		...
		System.out.println("Running benchmark on M/GEU using rnd distributions");
		System.out.println(no_iterations + " iterations for each sample (" + data.size() + " samples)");
		...
		for (double prefactor:prefactors) {
			...
			// already write something
			Utilities.writeEntries(all_iterations, "perf_all_iterations_approx3.txt.gz");
			Utilities.writeEntries(sample_construction, "sample_construction_approx3.txt.gz");
		}
