...
public class wDACO {
	...
	private Map<String, Double> protein_abundance;
	...
	public wDACO(ConstructedNetworks constr, int number_of_threads, int max_depth_of_search, double pair_building_threshold, double probability_cutoff, PrintStream verbose) {
		if (!constr.containsAbundanceData())
			throw new IllegalArgumentException("ConstructedNetwork object needs to incorporate abundance data. Please generate the input data with PPIXpress version 1.14 or newer.");
		
		this.ddi = constr.getDDIN();
		this.ppi = constr.getPPIN();
		this.protein_abundance = determineProteinAbundance(constr);
	...
	/**
	 * Maps proteins over transcripts/genes to abundances
	 * @param constr
	 * @return
	 */
	private Map<String, Double> determineProteinAbundance(ConstructedNetworks constr) {
		Map<String, Double> protein_abundance = new HashMap<>();
		
		for (Entry<String, String> entry:constr.getProteinToAssumedTranscriptMap().entrySet()) {
			String protein = entry.getKey();
			String transcript = entry.getValue();
			protein_abundance.put(protein, (double) constr.getTranscriptAbundanceMap().get(transcript));
		}
