...
public class RewiringDetector2 {
	...
	private Map<String, Double> g1_P_rews = new HashMap<>();
	private Map<String, Double> g2_P_rews = new HashMap<>();
	private Map<String, Double> inter_group_P_rews = new HashMap<>();
	...
	public RewiringDetector2(Map<String, ConstructedNetworks> group1, Map<String, ConstructedNetworks> group2, double FDR, int no_threads) {
		this.group1 = group1;
		this.group2 = group2;
		this.FDR = FDR;
		this.no_threads = no_threads;
		
		this.determineCommonDenominator();
		this.determineInGroupDifferences();
		this.determineGroupwiseDifferences();
		this.assessRewiring();
	}
	
	private void determineCommonDenominator() {
		Set<StrPair> all_IAs = new HashSet<>();
		
		for (ConstructedNetworks cn:group1.values())
			all_IAs.addAll(cn.getPPIN().getInteractionsFast());
		for (ConstructedNetworks cn:group2.values())
			all_IAs.addAll(cn.getPPIN().getInteractionsFast());
		
		this.denominator = (double) all_IAs.size();
	}
	
	private void determineInGroupDifferences() {
		
		List<PPIComparatorTask> comparison_calculations = new LinkedList<>();
		
		// prepare calculations for group1
		for (String sample1:this.group1.keySet())
			for (String sample2:this.group1.keySet())
				if (sample1.compareTo(sample2) == 1)
					comparison_calculations.add(new PPIComparatorTask(sample1, sample2, this.group1.get(sample1).getPPIN(), this.group1.get(sample2).getPPIN(), this.g1_P_rews));
		
		// prepare calculations for group2
		for (String sample1:this.group2.keySet())
			for (String sample2:this.group2.keySet())
				if (sample1.compareTo(sample2) == 1)
					comparison_calculations.add(new PPIComparatorTask(sample1, sample2, this.group2.get(sample1).getPPIN(), this.group2.get(sample2).getPPIN(), this.g2_P_rews));
		...
		try {
			for (Future<Object[]> f:es.invokeAll(comparison_calculations)) {
				f.get();// P_rews are set automatically
			}
		} catch (Exception e1) {
			System.err.println("Problem during assessment of in-group differences.");
			e1.printStackTrace();
			System.exit(1);
		}
		es.shutdown();
	...
	private void determineGroupwiseDifferences() {
		...
		for (String sample1:this.group1.keySet())
			for (String sample2:this.group2.keySet())
				comparison_calculations.add(new PPIComparatorTask(sample1, sample2, this.group1.get(sample1).getPPIN(), this.group2.get(sample2).getPPIN(), this.inter_group_P_rews));
	...
	private void assessRewiring() {
		...
		double P_rew = Utilities.getMean(this.inter_group_P_rews.values());
		double g1_P_rew = Utilities.getMean(this.g1_P_rews.values());
		double g2_P_rew = Utilities.getMean(this.g2_P_rews.values());
		P_rew = Math.max(P_rew, Math.max(g1_P_rew, g2_P_rew));
	...
	public Map<String, Double> getG1P_rew() {
		return g1_P_rews;
	}
	
	public Map<String, Double> getG2P_rew() {
		return g2_P_rews;
	}
	
	public Map<String, Double> getInterGroupP_rew() {
		return inter_group_P_rews;
	}
	...
	class PPIComparatorTask implements Callable<Object[]> {
		...
		private PPIN ppin1;
		private PPIN ppin2;
		private Map<String, Double> P_rew;
		
		public PPIComparatorTask(String sample1, String sample2, PPIN ppin1, PPIN ppin2, Map<String, Double> P_rew) {
		...
		public Object[] call() throws Exception {
			...
			this.P_rew.put(sample1 + "-" + sample2,   (added_interactions.size() + lost_interactions.size()) / denominator);
