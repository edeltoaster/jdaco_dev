...
public class build_diff_networks2 {
	...
	public static void process(String network_folder, String results_folder) {
		...
		for (String[] s:relations) {
			...
			RewiringDetector2 rd = new RewiringDetector2(g1, g2, FDR, 4);
			...
			double g1_P_rew_rounded = (double) Math.round( Utilities.getMean(rd.getG1P_rew().values() ) * 1000d) / 1000d;
			double g2_P_rew_rounded = (double) Math.round( Utilities.getMean(rd.getG2P_rew().values() ) * 1000d) / 1000d;
			double P_rew_rounded = (double) Math.round( Utilities.getMean(rd.getInterGroupP_rew().values() ) * 1000d) / 1000d;
			System.out.println(state1 + ":" + g1_P_rew_rounded);
			System.out.println(state2 + ":" + g2_P_rew_rounded);
			System.out.println(rd.getInterGroupP_rew().size()  + " comparisons, " + "P_rew: " + P_rew_rounded + ", " + rd.getInteractionReasonsMap().size() + " dIAs" );
